...
"Make plot showing radius, speed, mass, momentum per SN and kinetic energy efficiency as a function of time for different models in a uniform medium."
function create_figure_uniform(models::Dict{String, Dict{Symbol, Vector{Float64}}}, model_names::Vector{String}, 
                               names::Dict{String, String}, c_model::Dict{String, String}, tlim::Vector{<:Real})
    # create figure & axes
    ...
    for model in model_names
        ...
        axs[1,3].plot(models[model][:t], 4π * models[model][:M], color=c_model[model], linewidth=3, zorder=1)
        axs[2,1].plot(models[model][:t], 4π * models[model][:p], color=c_model[model], linewidth=3, zorder=1)
    ...
    axs[1,3].set_ylabel(L"\mathrm{Mass \ [M_{\odot}]}", fontsize=fontsize)
    axs[2,1].set_xlabel(L"\mathrm{Time \ [Myr]}", fontsize=fontsize)
    axs[2,1].set_ylabel(L"\mathrm{Momentum \ per \ SN \ [M_{\odot} \ km/s]}", fontsize=fontsize)
    axs[2,2].set_xlabel(L"\mathrm{Time \ [Myr]}", fontsize=fontsize)
    axs[2,2].set_ylabel(L"\mathrm{Kinetic \  Energy \ Efficiency}", fontsize=fontsize)
...
"Add lines for radius, speed, mass, momentum per SN and kinetic energy efficiency as a function of time for different models in a uniform medium to existing figure."
function plot_reference_models!(axs, models::Dict{String, Dict{Symbol, Vector{Float64}}}, model_names::Vector{String}, c_model::Dict{String, String}; 
                                solid_angle::Real=1.0, update_legend::Bool=true)
    # plot reference model results
    for model in model_names
        axs[1,1].plot(models[model][:t], models[model][:r], color=c_model[model], linestyle=":", linewidth=3, zorder=0)
        axs[1,2].plot(models[model][:t], models[model][:v], color=c_model[model], linestyle=":", linewidth=3, zorder=0)
        axs[1,3].plot(models[model][:t], solid_angle * models[model][:M], color=c_model[model], linestyle=":", linewidth=3, zorder=0)
        axs[2,1].plot(models[model][:t], solid_angle * models[model][:p], color=c_model[model], linestyle=":", linewidth=3, zorder=0)
        axs[2,2].plot(models[model][:t], models[model][:f_kin], color=c_model[model], linestyle=":", linewidth=3, zorder=0)
    end

    if update_legend
        # legend items
        axs[2,3].plot([NaN], [NaN], color="black", label=" Uniform Medium", linestyle=":", linewidth=3, zorder=0)
        axs[2,3].legend(loc = "center left", frameon=false, handlelength=1, fontsize=0.7fontsize)
    end
end